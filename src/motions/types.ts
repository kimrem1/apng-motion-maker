...
export interface PresetEmission {
   ...
   *
   * - 배열이 있으면 그것이 레이어 이펙트 스택 전체다. 호출부는 layer.effects 를
  ...
  perspective?: number
  /**
   * 회전과 확대가 도는 축. [0,1] 두 개이고 [0.5, 0.5] 가 한가운데다.
   *
   * 경첩 계열만 쓴다. 문이 도는 축은 문 한복판이 아니라 경첩이 달린 변이다.
   * 회전에 이동을 더해 흉내낼 수는 없다. 원근이 걸리면 각 점의 w 가 달라져서,
   * 원근 나눗셈 뒤에 어긋난 자리를 평행이동 하나로 되돌릴 수 없기 때문이다.
   *
   * 가리기와 같은 규칙이다. 값을 내지 않은 프리셋으로 갈아타면 한가운데로 돌아간다.
   * 사용자가 인스펙터에서 손으로 옮겨 둔 기준점은 살아남는다 (motions/merge.ts).
   */
...
export interface MotionPreset {
   ...
   * 이 프리셋은 점멸을 만든다.
   *
   * emit 이 내는 code:'flashWarning' 안내와 항상 같은 값이다. 안내는 적용 시점에
  ...
  /**
   * 글자 레이어에만 뜻이 있다. 갤러리가 다른 레이어에서는 감춘다.
   *
   * 글자 등장은 대부분 이미지와 도형에도 그대로 걸린다(오브제를 글자 한 개짜리 글
   * 상자로 계산한다). 예외는 글자 순번이 홀수냐 짝수냐로 갈리는 모양뿐이다.
   * 오브제는 언제나 0 번이라 "좌우 번갈아" 가 그냥 "왼쪽에서" 가 되어, 이름과 다르게
   * 움직이는 유령 항목이 된다. 감추는 편이 정직하다.
   */
  textOnly?: boolean
  /**
   * 원본이 클수록 결과가 또렷하다.
