 ...
 *
 * 레지스트리와 주고받는 값의 모양은 아래 두 어댑터에만 가둔다.
 *
 *   toEmitContext()   아는 값을 넉넉히 담아 넘긴다 (여분 필드는 무시된다)
 *   readEmission()    결과에서 tracks / modifiers / durationFrames / loop 만 읽는다
 *
 * 프리셋이 새 필드를 내기 시작하면 이 두 함수만 고치면 된다.
 */
 ...
 *
 * 프레임이 아니라 초로 들고 있어야 한다. "지금 프레임 x 직전 속도" 로 되짚으면
 * 두 군데서 틀린다. 프레임은 상한 120 에 잘리므로 잘린 뒤에는 곱셈으로 되돌아오지
 * 않고(현재 120 / 직전 속도 0.1 이면 기준선이 6배 틀어진다), fps 가 바뀌면 같은
 * 프레임 수가 다른 시간을 뜻하게 된다.
 *
