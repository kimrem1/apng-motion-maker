 ...
 *
 * 세기(strength)는 언제나 움직임의 크기에만 작용한다. 가리기 계열에서는 경계선의
 ...
 * 그림이 절반만 드러난 채로 끝난다" 가 된다. 그래서 세기는 함께 안착하는 배율에
 ...
 *
 * rotateX 는 가로축을 중심으로 돌므로 위아래 경첩이 쓰고, rotateY 는 세로축이라
 ...
 * '종이 펼치며 등장' 과 무엇이 다른가
 *
 * 저쪽은 한복판을 축으로 눕혔다 세우고, 어긋나는 자리를 세로 이동으로 어림해
