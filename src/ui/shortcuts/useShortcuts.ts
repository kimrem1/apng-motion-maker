 ...
 *
 *   1. 다른 모달이 떠 있으면 전부 잠근다. `[aria-modal="true"]` 가 화면에 있는데
 *      우리 오버레이가 아니면 그 모달이 키보드의 주인이다.
 *   2. 우리 오버레이(팔레트/도움말)가 떠 있으면 overlaySafe 만 통과시킨다.
 *   3. 입력 필드 포커스면 수식자 없는 단일 키를 전부 끈다. 그리고 Ctrl+A/C/V/X/Z 처럼
 *      텍스트 편집 기본 동작이 필요한 조합은 브라우저에 양보한다.
 *   4. 위젯 포커스면 그 위젯이 쓰는 키를 양보한다. 버튼에 포커스가 있을 때 Space 를
 ...
 * 단축키를 컴포넌트에서 따로 바인딩해도 마찬가지다. 실행취소가 두 칸씩 건너뛴다.
 */
