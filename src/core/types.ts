 ...
 *
 * 이 두 상수는 여기 한 곳에만 둔다. 같은 숫자를 프리셋 쪽에 한 벌 더 두면
 * 그 사본이 남아 있는 한 여기를 바꿔도 화면이 그대로다.
 *
...
export interface TimelineConfig {
  ...
  loop: LoopSpec
  /**
   * 사용자가 길이를 직접 입력해 못박았다.
   *
   * 이 표시가 있으면 모션/도형의 속도 노브는 전체 길이와 fps 를 건드리지 못한다.
   * 속도는 그 길이 **안에서** 모션이 얼마나 빨리 도는지만 정한다. 이 표시가 없으면
   * 예전처럼 속도가 길이를 정한다 (EASY 의 "재생 시간" 슬라이더가 그 경로다).
   *
   * true 일 때만 키를 만든다. false 가 저장 파일에 남으면 왕복 JSON 이 달라진다.
   */
...
export type RevealMode =
   ...
   *
   * clock 과 다르다. clock 은 한가운데를 돌고 한 바퀴(360도)를 채우지만,
   * 이쪽은 아래 변의 가운데를 돌고 반 바퀴(180도)만 쓴다. 부채와 아치는
...
export interface CharAnimSpec {
   ...
   * 것처럼 보인다. 흔들어도 진행률 1 에서는 전부 제자리라는 계약은 지킨다.
   */
  jitter: number
...
export interface Layer {
   ...
   * parentId 와 무엇이 다른가
   *
   * parentId 는 깊이감이다. 이동만, 그것도 parallaxFactor 를 곱해서 물려받는다.
   * 배경이 인물보다 조금만 움직이는 그림을 만드는 것이 전부다.
   *
   * folderId 는 담김이다. 폴더의 이동 / 회전 / 배율 / 기울임 / 투명도 / 구간이
   ...
   * 레이어의 알파라서 반투명한 가장자리는 반투명하게 잘린다.
   *
   ...
   *
   * 컷 편집의 토대다. 컷은 별도의 문서가 아니라 한 타임라인 위의 구간이고,
  ...
  containScale?: number
  /**
   * 이 레이어의 움직임만 몇 배 빠르게 돌릴 것인가. 없거나 1 이면 문서와 같은 속도다.
   *
   * 전체 길이를 건드리지 않는 속도 노브다
   *
   * 프리셋 갤러리의 속도 슬라이더는 **문서 전체 길이**를 정한다 (motions/apply.ts 의
   * targetSec). 그래서 오브제 하나를 빠르게 하려고 그 슬라이더를 올리면 프레임 수와
   * 재생 시간이 통째로 줄고, 같은 화면의 다른 오브제까지 함께 빨라진다. 이 필드는
   * 그 반대다. 문서의 fps 도 durationFrames 도 한 글자도 바뀌지 않고, 이 레이어만
   * 자기 모션을 한 문서 길이 안에서 N 번 돈다.
   *
   * 왜 정수인가
   *
   * 반복 재생의 이음새가 그것을 요구한다. 한 주기가 durationFrames / N 이면 문서
   * 끝(프레임 N*period = durationFrames)에서 주기도 정확히 0 으로 떨어진다. N 이
   * 정수가 아니면 마지막 주기가 잘려서, 지금까지 이음새가 없던 모션에 매 바퀴
   * 값 도약이 새로 생긴다. 모디파이어도 같다. sine 은 정수 주기여야 t=0 과 t=1 이
   * 값과 기울기 둘 다 이어진다 (motions/generators.ts).
   *
   * 왜 굽지 않고 필드로 두는가
   *
   * 트랙을 실제로 이어 붙이면(도형 세트의 tileTrack 이 그렇게 한다) 되돌리는 길이
   * 실행취소밖에 없다. 노브는 왕복할 수 있어야 노브다. 값으로 들고 있으면 1 로
   * 되돌리는 것이 곧 원상복구이고, 트랙과 키프레임은 사용자가 만든 그대로 남는다.
   *
   * 대신 타임라인과 그래프 에디터는 키를 문서 좌표에 그대로 그린다. 즉 화면에는
   * 한 주기가 보이고 재생은 N 번 돈다. 그 사실을 레이어 줄과 인스펙터가 문구로 알린다.
   *
 ...
 * 레이어 모션 배수의 범위.
 *
 * 상한 12 는 최소 주기 2프레임과 프레임 상한 120 에서 나온 실용값이다. 그보다 올려도
 * 24프레임짜리 문서에서는 주기가 2프레임 아래로 내려가 정수 격자에서 뭉개진다.
 * 실제 상한은 durationFrames / 2 로 한 번 더 조인다 (core/evaluate.ts effectiveRepeat).
 */
...
export interface PresetRef extends PresetOwnershipRecord {
   ...
   * 왕복할 때마다 오차가 다시 들어와 길이가 계속 늘어난다.
   * 속도 1 -> 0.5 -> 2 -> 0.5 -> 1 을 한 번 왕복하면 1.20초가 1.28초가 된다.
   *
   ...
   * 속도를 되돌려도 길이가 원래대로 돌아오지 않는다. 0.92초짜리가 0.88초로 어긋난다.
   *
