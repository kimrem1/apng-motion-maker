 ...
 * 무엇을 기준으로 자르는가
 *
 * 아래 레이어의 알파다. 색이 아니다. 그래서 반투명한 가장자리는 반투명하게
 * 잘리고, 글자의 안티에일리어싱이 그대로 살아난다.
 *
 * 폴더도 자를 수 있고 폴더로 자를 수도 있다
 *
 * 폴더가 잘리는 쪽이면 폴더 안의 그림 전체가 한 장처럼 잘린다. 폴더가 밑판이면
 * 폴더 안의 그림 전체가 마스크가 된다. 그래서 "도형 여러 장으로 만든 모양"
 * 안에만 사진을 채우는 것이 된다.
 *
 ...
 *
 * 여러 장이 연달아 켜져 있으면 전부 같은 밑판을 쓴다. 위에서부터 물려받는
 ...
 *
 * 폴더가 아니면 자기 자신이다. 폴더면 안에 든 것의 끝이다. 폴더 식구는 폴더
...
export function clipBaseIndexes(layers: readonly ClipInput[]): number[] {
  ...
  for (let i = 0; i < layers.length; i += 1) {
     ...
     *   - 폴더 아이디가 다르면 앞 형제(폴더)의 안쪽이다. 형제가 아니므로 건너뛴다.
