      ...
      for (let i = 0; i < count; i += 1) {
        ...
        layers.push({
          ...
          tracks: [
            ...
            /*
             * 한 바퀴는 span 에서 끝난다. end(= span - 1) 가 아니다.
             *
             * 360 도와 0 도는 같은 그림이라 마지막 출력 프레임에 값이 겹치지 않는다.
             * end 에 두면 마지막 한 프레임이 각도를 붙잡아 주기마다 회전이 한 번
             * 멈칫한다. 같은 파일 꽃잎(ambient.petals)이 그 이유를 적어 두고 span 을
             * 쓰는데 색종이만 반대였다. 좌우 흔들림은 대칭이라 티가 안 나고
             * 회전에서만 드러난다.
             */
            track(
              'rotate',
