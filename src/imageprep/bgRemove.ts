 ...
 * 단색 배경 제거.
 *
 * 14.A1 에서 주 용도가 투명 배경 스티커로 확정됐다. 그런데 사용자가 가진 소재는
...
/**
 * 색 거리. **RGB 유클리드가 아니라 YCbCr 크로마 거리 + 휘도 가중**을 쓴다.
 *
 ...
 * 결과는 1px 짜리 흰 테두리다. 14.A1 이 지목한 바로 그 결함이다.
 *
 ...
 *
 * 왜 필요한가. 우리 파이프라인은 straight alpha 를 유지한다(14.A1). GPU 이중선형
...
export async function removeBackground(
  ...
  //
  //    m 은 절대 알파가 아니라 **원본 알파 대비 남은 비율**이다. 원본이 이미
